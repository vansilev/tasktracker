...
new #[Layout('components.tasks-layout')] class extends Component
{
    ...
    public function save(BillingItemService $items): void
    {
        ...
        try {
            $item = $items->save(null, $this->payload());
        ...
        $this->redirect(route('billing.show', $item), navigate: true);
    }
    ...
    private function rulesForStep(int $step): array
    {
        return match ($step) {
            ...
            default => [
                ...
                'cardLast4' => [$this->paymentMethod === BillingPaymentMethod::Card->value ? 'nullable' : 'nullable', 'regex:/^\d{4}$/'],
...
<div>
    <x-slot name="title">{{ __('billing.create') }}</x-slot>
