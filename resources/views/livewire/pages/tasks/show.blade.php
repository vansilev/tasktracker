...
new #[Layout('components.tasks-layout')] class extends Component
{
    ...
    public function addComment(TaskService $tasks, TaskAttachmentService $attachments, SettingsService $settings): void
    {
        ...
        $this->resetQuotes();
        $this->reloadComments();
        $this->js('queueMicrotask(() => document.querySelector("[data-ui=message-scroller]")?.scrollTo({ top: 1e9 }))');
    }
            ...
            <div class="min-w-0">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    <h1 class="text-lg font-semibold text-gray-900">
                        <x-task-number :task="$task" class="text-lg font-semibold text-gray-400 hover:text-indigo-700" />
                        · {{ $task->title }}
                    </h1>
            ...
            <form wire:submit="saveEdit" class="space-y-4 max-w-3xl">
                ...
                @if ($canAssign)
                    <div>
                        ...
                        <x-combobox
                            class="mt-1"
                            wire:model.live="editAssigneeDepartmentId"
                    ...
                    <div>
                        ...
                        <x-combobox
                            class="mt-1"
                            wire:key="edit-assignee-{{ $editAssigneeDepartmentId }}"
                            wire:model="editAssigneeId"
                            :options="$assignees->map(fn ($u) => ['value' => $u->id, 'label' => $u->name])->all()"
                ...
                <x-card padding="p-0" class="overflow-hidden">
                    <div class="flex items-center justify-between gap-3 border-b border-gray-100 px-5 py-4">
                        ...
                        @if ($task->comments->isNotEmpty())
                            <span class="text-xs tabular-nums text-gray-400">{{ $task->comments->count() }}</span>
                        @endif
                    </div>
                    @php
                        $thread = $task->comments->sortBy('created_at')->values();
                    @endphp
                    <div
                    ...
                    >
                        @forelse ($thread as $comment)
                            ...
                            />
                        @empty
                            <p class="py-8 text-center text-sm text-gray-500">{{ __('No comments yet.') }}</p>
                        @endforelse
                    </div>
                    @if ($canComment)
                    <form wire:submit="addComment" class="space-y-2 border-t border-gray-100 bg-gray-50/70 px-5 py-4"
                    ...
                    <div class="p-5 grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-4 items-start max-h-[calc(100vh-10rem)] overflow-y-auto">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <x-combobox
                                    class="mt-1"
                                    wire:model.live="subtaskDepartmentId"
                                    :disabled="count($subtaskDepartments) === 1"
                                    :options="$subtaskDepartments->map(fn ($dept) => ['value' => $dept->id, 'label' => $dept->name])->all()"
                            ...
                            <div>
                                ...
                                <x-combobox
                                    class="mt-1"
                                    wire:key="subtask-assignee-{{ $subtaskDepartmentId }}"
                                    wire:model="subtaskAssigneeId"
                                    :options="collect([['value' => null, 'label' => __('— Auto (head / queue) —')]])->concat($subtaskUsers->map(fn ($u) => ['value' => $u->id, 'label' => $u->name]))->all()"
