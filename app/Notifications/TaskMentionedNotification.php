...
class TaskMentionedNotification extends Notification implements ShouldQueue
{
    ...
    {
        /** @var User $notifiable */
        $line = $this->mentionedLine($this->task, $this->actor, $this->commentExcerpt, $notifiable);
    ...
    public function toTelegram(object $notifiable): TelegramMessage
    {
        ...
        return TelegramMessage::create()
            ->normal()
            ->content($this->mentionedLine($this->task, $this->actor, $this->commentExcerpt, $notifiable))
            ->button(
                $this->localizedLine($notifiable, 'notification.open_task'),
                $this->taskUrl($this->task),
            );
